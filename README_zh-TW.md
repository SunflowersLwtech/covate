...
Covate 採用 **open-core（開放核心）** 模式。本倉庫的 MCP server **永久免費開源（MIT）**——
本地執行、無需帳號。可選的 **[Covate 學習平台](https://covate-platform.vercel.app)** 是付費雲端層，
把 Covate 已經捕獲的學習時刻沉澱為長期、可衡量的成長：
...
現處於搶先體驗階段 —— **[加入等候名單 →](https://covate-platform.vercel.app)**。不使用它，MCP 仍完全可用。
