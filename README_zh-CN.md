...
Covate 采用 **open-core（开放核心）** 模式。本仓库的 MCP server **永久免费开源（MIT）**——
本地运行、无需账号。可选的 **[Covate 学习平台](https://covate-platform.vercel.app)** 是付费云端层，
把 Covate 已经捕获的学习时刻沉淀为长期、可衡量的成长：
...
现处于抢先体验阶段 —— **[加入等候名单 →](https://covate-platform.vercel.app)**。不使用它，MCP 仍完全可用。
